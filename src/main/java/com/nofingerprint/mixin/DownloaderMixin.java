package com.nofingerprint.mixin;

import com.llamalad7.mixinextras.injector.ModifyExpressionValue;
import com.llamalad7.mixinextras.sugar.Local;
import com.nofingerprint.NofingerPrint;
...
import static com.nofingerprint.NofingerPrint.mc;
...
public class DownloaderMixin {
    ...
    private Path hookResolve(Path original, @Local(argsOnly = true) UUID id) {
        ...
        if (directory == null) return original;

        UUID accountId = mc.getSession().getUuidOrNull();
        if (accountId == null) {
            NofingerPrint.LOGGER.warn(
                    "Failed to change resource pack download directory: account id is null."
            );
            return original;
        }
